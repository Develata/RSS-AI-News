...
- 解析失败显式拆分为 `AiError::InvalidJson` / `MissingField { field }` / `InvalidFieldValue { field, reason }` 三类（见 [error-and-observability §2.3](./error-and-observability.md)），不静默 fallback

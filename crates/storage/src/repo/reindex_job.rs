...
//! 分发到 sqlite_*/pg_* 私有 helper。除两条 claim 路径外，SQL 跨方言完全等价。
//!
//! **claim 路径的 §6.4 契约**：PG 必须 `FOR UPDATE SKIP LOCKED`，否则多 worker
//! 并发 claim 同一 pending 池时会串行化等待 row lock，等价单 worker。SQLite 整库
//! 写锁本身阻塞并发，不需要 SKIP LOCKED 也不支持 `FOR UPDATE`。两侧 SQL 因此
//! 分叉，由 `CLAIM_PENDING_SQLITE_SQL` / `CLAIM_PENDING_PG_SQL` 明确区分。
...
pub trait ReindexJobRepository: Send + Sync {
    ...
    ///
    /// **PG 契约**（§6.4）：子查询使用 `FOR UPDATE SKIP LOCKED`，让多 worker
    /// 并发抢同一池时各自拿到不同候选；SQLite 无此语法且整库写锁本身串行化。
...
impl ReindexJobRepo {
    ...
    /// W11-P3-C-2：PG 入口；旧 `new(SqlitePool)` thin wrapper 保留兼容。
...
const INSERT_RULE_VERSION_PENDING_SQL: &str = r#"
INSERT INTO rule_versions (kind, version_tag, description, payload_sha256, status)
VALUES ($1, $2, $3, $4, 'pending')
RETURNING id
"#;

const INSERT_REINDEX_JOB_PENDING_SQL: &str = r#"
...
const COMPLETE_WITHOUT_CLAIM_SQL: &str = r#"
UPDATE reindex_jobs
SET state = 'completed',
    lease_owner = NULL,
...
const FINISH_REINDEX_UPDATE_JOB_SQL: &str = r#"
UPDATE reindex_jobs
SET state = 'completed',
    lease_owner = NULL,
    lease_expires_at = NULL,
...
const FINISH_REINDEX_DEMOTE_ACTIVE_SQL: &str = r#"
...
const FINISH_REINDEX_PROMOTE_PENDING_SQL: &str = r#"
UPDATE rule_versions
SET status = 'active'
...
/// 整库写锁本身串行化并发）。
const CLAIM_PENDING_SQLITE_SQL: &str = r#"
UPDATE reindex_jobs
SET state = 'running',
    lease_owner = $1,
    lease_expires_at = $2,
...
const CLAIM_PENDING_PG_SQL: &str = r#"
UPDATE reindex_jobs
SET state = 'running',
    lease_owner = $1,
    lease_expires_at = $2,
...
const CLAIM_BY_ID_SQLITE_SQL: &str = r#"
UPDATE reindex_jobs
SET state = 'running',
    lease_owner = $1,
    lease_expires_at = $2,
...
/// （与"已被 claim"语义一致）。
const CLAIM_BY_ID_PG_SQL: &str = r#"
UPDATE reindex_jobs
SET state = 'running',
    lease_owner = $1,
    lease_expires_at = $2,
...
const ASSERT_LEASE_HELD_SQL: &str = r#"
UPDATE reindex_jobs
SET updated_at = $1
WHERE id = $2 AND state = 'running' AND lease_owner = $3
"#;

const ADVANCE_TO_COMPLETED_SQL: &str = r#"
UPDATE reindex_jobs
SET state = 'completed',
    lease_owner = NULL,
    lease_expires_at = NULL,
...
const RECLAIM_EXPIRED_LEASES_SQL: &str = r#"
UPDATE reindex_jobs
SET state = 'pending',
    lease_owner = NULL,
    lease_expires_at = NULL,
